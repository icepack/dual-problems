...
The Stokes equations with a power law rheology (commonly referred to as Glen's law \citep{glen1958law}) is the most common choice for modelling glaciers like viscous fluids \citep{greve2009dynamics}. Solving the Stokes equations in a three-dimensional setting will generally require a high computational cost. For this reason, it is common to simplify these equations by exploting the fact that glacier flow has a small ratio of thickness to horizontal length scale and that certain components of the strain rate tensor become negligible under certain circumstances. Here, we shall focus on the \emph{shallow stream/shelf approximation} (SSA), which assumes the flow of ice to be approximately invariant in the vertical direction (a plug flow); as a result, for a Cartesian coordinate system where the $z$-axis corresponds with the vertical direction of the glacier, the $x-z$ components of the strain rate tensor are much smaller than the $x-x$, $x-y$, and $y-y$ components. The equations of motion under the action of gravity are then derived by removing the pressure and integrating the momentum balance equation depthwise, as described in \cite[Section 6]{greve2009dynamics}. The equations of motion are then solved in a two-dimensional domain $\Omega$ whose boundary we denote by $\partial \Omega$.

In this setting, the unknowns are the depthwise averaged ice velocity $u$, the \emph{membrane stress tensor} $M$, a rank-2 tensor with units of stress or energy density that results from applying the above approximations to the 3D deviatoric stress tensor, the basal shear stress $\tau$, and the ice thickness $h$. Conservation of momentum for an ice sheet grounded on the bedrock is 
\begin{equation}
    \nabla\cdot (hM) + \tau - \rho gh\nabla s = 0,
    \label{eq:membrane-stress-conservation}
\end{equation}
where $s$ is the surface elevation, $\rho$ is the ice density, and $g$ the gravitational acceleration. In addition to \eqref{eq:membrane-stress-conservation}, we need to know a constitutive relation between the membrane stress tensor and the depth-averaged strain rate tensor
\begin{equation}
...
To close the system of equations, the following depth-averaged statement for the conservation of mass can be derived:
\begin{equation}
	\frac{\partial h}{\partial t} + \nabla \cdot (hu) = a,
	\label{eq:conservation-mass}
\end{equation}
where $a$ is the rate at which ice accumulates due to snowfall or melting. 

For modelling a floating ice shelf, we remove the friction term in \eqref{eq:membrane-stress-conservation}. Moreover, momentum balance in the vertical direction leads to the hydrostatic flotation condition
\begin{equation}
	s = \delta h,
	\label{eq:flotation-condition}
\end{equation}
where $\delta = 1 - \rho/\rho_w$, with $\rho_w$ denoting the density of the ocean water. As a result, conservation of momentum is given by
\begin{equation}
	\nabla\cdot (hM) = \rho g \delta h \nabla h.
\end{equation}

Marine ice sheets flow from the continent and into the ocean, where they go afloat. If we are to model a marine ice sheet with the SSA, we must distinguish between a grounded and a floating region. This results in a free boundary problem where ice goes float once the flotation condition \eqref{eq:flotation-condition} is satisfied. The boundary separating grounded from floating ice is known as the grounding line $x_g$. The possibility of a marine ice sheet isntability that could dramatically increase the rate of discharge of ice into the ocean has led to a substantial amount of research into grounding line dynamics \citep{schoof2007mis, durand2009mis, favier2012mis}.

Finally, we need to supply a set of boundary conditions for the problem to be well-posed. These will be stated when introducing each of the problems considered in this paper.
...
Here we briefly review these concepts as they pertain to the SSA momentum balance model \eqref{eq:membrane-stress-conservation} for a grounded ice sheet. For concreteness, in this section and the next we assume the following boundary conditions. We partition the boundary $\partial\Omega$ into an inflow region $\Gamma_i$ and the glacier terminus $\Gamma_t$. At $\Gamma_i$, we assume that the velocity is known from observations.
This is a Dirichlet boundary condition.
At $\Gamma_t$, the membrane stresses at the cliff face are balanced by pressures from any proglacial water body:
\begin{equation}
...
If we assume that the ice thickness $h$ is known, we can then combine equations \eqref{eq:membrane-stress-conservation}, \eqref{eq:strain-rate}, \eqref{eq:constitutive-relation}, and \eqref{eq:sliding-law} to arrive at a second-order, nonlinear system of partial differential equations for $u$. This equation can be derived as the optimality conditions of an action functional. We can see this with the aid of the antiderivatives of the membrance and basal stresses. 
